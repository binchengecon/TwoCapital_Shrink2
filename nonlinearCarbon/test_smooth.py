...
# max = 10
    ...
    for impulse in pulsearray:
        ...
        Tvmid = pd.Series(Tvmid)
        Tvmid = Tvmid.rolling(window=9999)
        Tvmid = Tvmid.mean()
                
        Cv = pd.Series(Cv)
        Cv = Cv.rolling(window=9999)
        Cv = Cv.mean()
        ...
        if pathnum==0:
            ...
            axs[3].plot(tv, (Tvmid-TvmidBase)*1000/impulse, label=f"CarbonImpulse={impulse}")
        axs[3].set_xlabel('Time (year)')
        axs[3].set_ylabel('Degree Celsius')
        axs[3].set_title('Impulse Response of temperature anomaly per teratonne of Carbon')
        axs[3].grid(linestyle=':')
        axs[3].legend()        

        if pathnum==0:
            plt.tight_layout()
            plt.savefig(Figure_Dir+"Pulse="+file_name+"_smooth.pdf")
            plt.savefig(Figure_Dir+"Pulse="+file_name+"_smooth.png")
    ...
    plt.tight_layout()
    plt.savefig(Figure_Dir+"Pulse="+file_name+"_pulsetime="+str(year)+",pulsesize="+str(max)+"_smooth.pdf")
    plt.savefig(Figure_Dir+"Pulse="+file_name+"_pulsetime="+str(year)+",pulsesize="+str(max)+"_smooth.png")
    plt.close()


# Tvmid_S = pd.Series(Tvmid)
# Tvmid_SR = Tvmid_S.rolling(window=9999)
# Tvmid_SRmean = Tvmid_SR.mean()
# # plt.plot(tv,Tvmid,label="orig")
# # plt.legend("orig")
# plt.plot(tv,Tvmid_SRmean,label="new")
# # plt.legend("new")
# plt.legend()
# plt.savefig("./nonlinearCarbon/figure/test.png")
# plt.close()
