...
for i in range(len(Cebis)):
    if i == 0:
        ...
        Cebis[i] = max( Ca[i] - Ca[i-1], 0) +pulse
    else :
        Cebis[i] = max( Ca[i] - Ca[i-1], 0)
...
plt.tight_layout()
plt.savefig(Figure_Dir+f"Pulse="+file_name+"_pulsetime="+str(year)+",pulsesize="+str(pulse)+".pdf")
plt.savefig(Figure_Dir+f"Pulse="+file_name+"_pulsetime="+str(year)+",pulsesize="+str(pulse)+".png")
