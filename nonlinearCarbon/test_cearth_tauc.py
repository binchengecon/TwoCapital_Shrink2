...
def model(time, pulse, cearth, tauc):
...
for i in range(ceartharray):
    ...
    axs[0].plot(tvBase, TvmidBase,
                label=f"cearth={ceartharray[i]}, tauc={taucarray[i]}")
...
plt.savefig(Figure_Dir+"Pulse="+file_name+",cearth" +
            str(ceartharray)+",tauc="+str(taucarray)+".pdf")
plt.savefig(Figure_Dir+"Pulse="+file_name+",cearth" +
            str(ceartharray)+",tauc="+str(taucarray)+".png")
plt.close()
