...
def model(time, pulse, cod, wa, cearth, tauc):
...
ceartharray = np.linspace(0.3725, 10, 30)
taucarray = np.linspace(30, 1000, 30)

ceartharray_meshgrid, taucarray_meshgrid = np.meshgrid(ceartharray, taucarray)
...
for i in range(len(ceartharray)):
    for j in range(len(taucarray)):
        tvBase, TvmidBase, CvBase, TeBase, CcBase, file_name, cod, wa, cearth = model(
            0, 0, 1.65, 0.0045, ceartharray[i], taucarray[j])
        print(cearth)
        Tarray[i, j] = TvmidBase[int(210/600*len(tvBase)-1)]
        Carray[i, j] = CvBase[int(210/600*len(tvBase)-1)]
...
axs[0].scatter(ceartharray_meshgrid, taucarray_meshgrid, Tarray,
               label="cod={:.2f}, wa={:.4f}".format(cod, wa))
...
axs[1].scatter(ceartharray_meshgrid, taucarray_meshgrid, Carray,
               label="cod={:.2f}, wa={:.4f}".format(cod, wa))
...
plt.savefig(Figure_Dir+"Pulse="+file_name +
            ",cearth=0.3725-10"+"tauc=30-1000"+".pdf")
plt.savefig(Figure_Dir+"Pulse="+file_name +
            ",cearth=0.3725-10"+"tauc=30-1000"+".png")
plt.close()
...
axs[0].plot_surface(ceartharray_meshgrid, taucarray_meshgrid, Tarray,
                    label="cod={:.2f}, wa={:.4f}".format(cod, wa))
axs[0].set_xlabel('Cearth ')
axs[0].set_ylabel('Temperature (K)')
axs[0].set_title('Temperature Anomaly Dynamics T')
...
axs[1].plot_surface(ceartharray_meshgrid, taucarray_meshgrid, Carray,
                    label="cod={:.2f}, wa={:.4f}".format(cod, wa))
axs[1].set_xlabel('Cearth ')
axs[1].set_ylabel('Carbon (ppm)')
axs[1].set_title('Carbon Concentration Dynamics C')
...
plt.tight_layout()
plt.savefig(Figure_Dir+"Pulse="+file_name +
            ",cearth=0.3725-10"+"tauc=30-1000"+",surface"+".pdf")
plt.savefig(Figure_Dir+"Pulse="+file_name +
            ",cearth=0.3725-10"+"tauc=30-1000"+",surface"+".png")
plt.close()
