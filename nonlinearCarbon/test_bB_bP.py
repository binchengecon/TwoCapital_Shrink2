...
def model(time, pulse, bB, bP, cearth):
    ...
    return tv+1800, Tvmid, Cv, T+1800, Cc, file_name, bB, bP, cearth
...
bBarray = np.array((0.05, 0.04, 0.03))
bParray = np.array((0.05, 0.06, 0.07))
ceartharray = np.linspace(0.3725, 10, 20)
...
for i in range(len(bBarray)):

    Tarray = np.zeros(len(ceartharray))
    Carray = np.zeros(len(ceartharray))

    for j in range(len(ceartharray)):

        tvBase, TvmidBase, CvBase, TeBase, CcBase, file_name, bB, bP, cearth = model(
            0, 0, bBarray[i], bParray[i], ceartharray[j])

        Tarray[j] = TvmidBase[int(210/600*len(tvBase)-1)]
        Carray[j] = CvBase[int(210/600*len(tvBase)-1)]
    ...
    axs[0].plot(ceartharray, Tarray, label=f"bB={bB},bP={bP}")
    ...
    axs[1].plot(ceartharray, Carray, label=f"bB={bB},bP={bP}")
    ...
    # axs[2].plot(TeBase, CcBase*2.13, label=f"bB={bB},bP={bP}")
...
# axs[3].plot(tvBase, TvmidBase-TvmidBase, label=f"bB={bB},bP={bP}")
...
plt.tight_layout()
plt.savefig(Figure_Dir+"Pulse="+file_name+",bB=" +
            str(bBarray)+",bP="+str(bParray)+".pdf")
plt.savefig(Figure_Dir+"Pulse="+file_name+",bB=" +
            str(bBarray)+",bP="+str(bParray)+".png")
plt.close()
