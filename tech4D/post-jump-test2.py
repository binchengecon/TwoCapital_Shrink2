...
parser = argparse.ArgumentParser(description="Keywords for post jump model")
parser.add_argument("--linsol", default="eigen", help="Choose linear solver, could be 'eigen', 'petsc4py' or 'petsc'")
parser.add_argument("--scale", action="store_true")
parser.add_argument("--capmin", default=-1, type=float, help="Set the lower level of id ig")
...
# filename = open(linearsolver + "-" + str(args.capmin) + "-" + str(args.scale) + "{}".format(current_time.second), "w")
# filename.write("{}-{}".format(current_time.minute, current_time.second))
# exit()

csvfile = open(linearsolver + "-" + str(args.capmin) + "-" + str(args.scale) + "-" + "{:d}-{:d}-{:d}.csv".format(current_time.day, current_time.hour, current_time.minute), "w")
fieldnames = ["epoch", "iterations", "residual norm",  "PDE_Err", "FC_Err", "A_norm", "b_norm"]
...
while FC_Err > tol and epoch < max_iter:
    ...
    if epoch == 0:
        ...
        i_d[i_d < args.capmin] = args.capmin
        i_d[i_d > A_d] = A_d
        i_g = (1 / phi_g - mc / phi_g / dKg * Kg_max) * fraction + i_g * (1 - fraction)
        i_g[i_g < args.capmin] = args.capmin
        i_g[i_g > A_g] = A_g
        # nums = 0
        # converge = False
        # while not converge:
    ...
    if linearsolver == 'eigen' or linearsolver == 'both':
        ...
        LHS_co = out_eigen[3]
        RHS = out_eigen[4]
        # print(scipy.sparse.linalg.norm(LHS_co))
        # print(RHS.shape)
        # print(norm(RHS))
        norm_lhs = norm(LHS_co)
        # lhs_dense = LHS_co.toarray()
        # np.linalg.cond(lhs_dense)
        # inv_lhs = norm(inv(LHS_co), np.inf)
