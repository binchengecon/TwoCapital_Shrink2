...
filename =  "pre_damage_" + "xi_a_" + str(xi_a) + "_xi_d_" + str(xi_d) + "_xi_g_" + str(xi_g)  + '_{}'.format(current_time)
...
for gamma_3_i in gamma_3_list:
    res = pickle.load(open("post_jump_" + str(gamma_3_i), "rb"))
    v_i = res["v0"]
    Temp = res["Y"]
    id_2 = np.abs(Temp - y_bar).argmin()
    v_i_bar = np.zeros(K_mat.shape)
    v_i_bar[:, :, :] = v_i[:, id_2, :]
    v_post_list.append(v_i_bar)

v_post_i = np.array(v_post_list)
...
# Damage jump poission process intensity
r_1 = 1.5
r_2 = 2.5
J_d = r_1 * np.exp(r_2 / 2 * (Y_mat - y_bar_lower)**2) * (Y_mat > y_bar_lower)
...
while FC_Err > tol and epoch < max_iter:
    ...
    D -= xi_d * J_d * (np.sum(pi_d_o * np.exp(-v_post_i / xi_d), axis=0) - np.exp(- v0 / xi_d)) / np.exp(- v0 / xi_d)
