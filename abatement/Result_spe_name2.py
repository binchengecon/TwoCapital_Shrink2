...
def simulate_pre(
    grid = (), model_args = (), controls = (), initial=(np.log(85/0.115), 1.1, np.log(1/80)), 
    T0=0, T=40, dt=1/12,
    printing=False):
...
def graph2(psi_0_meshgrid,psi_1_meshgrid,Ig_initial = 1/120):
    ...
    def graph_solution_extraction(res):
        ...
        for k in range(len(psi_0_meshgrid_1d)):
            axs1[0].plot(res[k]["years"], (res[k]["x"]/(alpha*np.exp(res[k]["states"][:,0]))),label=r'$\psi_0=$'+str(psi_0_meshgrid_1d[k])+'$\psi_1=$'+str(psi_1_meshgrid_1d[k]),linewidth=7.0)
